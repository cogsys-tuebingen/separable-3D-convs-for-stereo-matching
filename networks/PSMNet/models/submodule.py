...
def convbn_3d(in_planes, out_planes, kernel_size, stride, pad, conv_type):

    if conv_type=='FwSC':
        conv = FwSC(in_channels=in_planes, out_channels=out_planes, kernel_size=kernel_size, stride=stride,
                     padding=pad)

        return nn.Sequential(conv, nn.BatchNorm3d(out_planes))
    elif 'FDwSC':
        conv = FDwSC(in_channels=in_planes, out_channels=out_planes, kernel_size=kernel_size, stride=stride,
                                    padding=pad)
