...
parser.add_argument('--model', default='stackhourglass',
                    help='select model')
parser.add_argument('--datapath', default='/data2/psmdata',
                    help='datapath')
parser.add_argument('--epochs', type=int, default=0,
                    help='number of epochs to train')
parser.add_argument('--loadmodel', default='./checkpoint/pretrained_sceneflow.tar',
                    help='load model')
...
all_left_img, all_right_img, all_left_disp, test_left_img, test_right_img, test_left_disp = lt.dataloader(args.datapath, test_list='./lists/sceneflow_test_select.list')
...
def test(imgL, imgR, disp_true, running_error_sum):
    ...
    error_dict = calc_error(tmp_out[:,:-4,:], disp_true, lb=0.001, ub=args.maxdisp)
    running_error_sum = {k: running_error_sum.get(k, 0) + error_dict.get(k, 0) for k in set(error_dict)}
    # ************************

    return loss.data.cpu(), running_error_sum
...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        print('epoch %d total training loss = %.3f, time= %.2f' % (epoch, total_train_loss / len(TrainImgLoader), time.time() - epoch_start))
