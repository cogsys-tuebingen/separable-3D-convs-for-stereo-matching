...
def dataloader(filepath, test_list):
    ...
    file_name= test_list
    print(f"Reading Test Images from file: {file_name}")
    f = open(file_name, 'r')
    ...
    test_left_img = [flying_path+ "/" + img[0: len(img) - 1] for img in filelist]
    test_right_img= [flying_path+ "/"+  img[0: len(img) - 14] + 'right/' + img[len(img) - 9:len(img) - 1] for img in filelist]
    test_left_disp= [flying_disp+ "/"+ img[0: len(img) - 4] + 'pfm' for img in filelist]
