...
from . import __convolutions__

class BasicConv(nn.Module):

    def __init__(self, in_channels, out_channels, deconv=False, is_3d=False, bn=True, relu=True, conv_type="FwSC", **kwargs):
        ...
        if is_3d:
            if deconv:
                ...
                self.conv = __convolutions__[conv_type](in_channels, out_channels, bias=False, **kwargs)
...
class Conv2x(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, deconv=False, conv_type= None, is_3d=False, concat=True, bn=True, relu=True):
        ...
        if deconv and is_3d:
            ...
            kernel = 3
        self.conv1 = BasicConv(in_channels, out_channels, deconv, is_3d, conv_type= conv_type , bn=True, relu=True, kernel_size=kernel, stride=2, padding=1)

        if self.concat:
            self.conv2 = BasicConv(out_channels*2, out_channels, False, is_3d, bn, relu, conv_type= conv_type , kernel_size=3, stride=1, padding=1)
        else:
            self.conv2 = BasicConv(out_channels, out_channels, False, is_3d, bn, relu, conv_type= conv_type , kernel_size=3, stride=1, padding=1)
...
class SGABlock(nn.Module):
    def __init__(self, conv_type,  channels=32, refine=False):
        super(SGABlock, self).__init__()
        self.refine = refine
        if self.refine:
            self.bn_relu = nn.Sequential(BatchNorm3d(channels),
                                         nn.ReLU(inplace=True))
            self.conv_refine = BasicConv(channels, channels, is_3d=True, conv_type=conv_type , kernel_size=3, padding=1, relu=False)
...
class CostAggregation(nn.Module):
    def __init__(self, conv_type, maxdisp=192 ):
        super(CostAggregation, self).__init__()
        self.maxdisp = maxdisp
        self.conv_type= conv_type

        self.conv_start = BasicConv(64, 32, is_3d=True, conv_type= conv_type , kernel_size=3, padding=1, relu=False)

        self.conv1a = BasicConv(32, 48, is_3d=True, conv_type= conv_type,  kernel_size=3, stride=2, padding=1)
        self.conv2a = BasicConv(48, 64, is_3d=True, conv_type= conv_type,  kernel_size=3, stride=2, padding=1)
        #        self.conv3a = BasicConv(64, 96, is_3d=True, kernel_size=3, stride=2, padding=1)

        self.deconv1a = Conv2x(48, 32, deconv=True, is_3d=True, conv_type= conv_type,  relu=False)
        self.deconv2a = Conv2x(64, 48, deconv=True, is_3d=True, conv_type= conv_type )
        #        self.deconv3a = Conv2x(96, 64, deconv=True, is_3d=True)

        self.conv1b = Conv2x(32, 48, is_3d=True, conv_type= conv_type )
        self.conv2b = Conv2x(48, 64, is_3d=True, conv_type= conv_type )
        #        self.conv3b = Conv2x(64, 96, is_3d=True)

        self.deconv1b = Conv2x(48, 32, deconv=True, is_3d=True, conv_type= conv_type,  relu=False)
        self.deconv2b = Conv2x(64, 48, deconv=True, is_3d=True, conv_type= conv_type )
        #        self.deconv3b = Conv2x(96, 64, deconv=True, is_3d=True)
        self.deconv0b = Conv2x(8, 8, deconv=True, is_3d=True, conv_type= conv_type )

        self.sga1 = SGABlock(conv_type=conv_type, refine=True)
        self.sga2 = SGABlock(conv_type=conv_type, refine=True)
        self.sga3 = SGABlock(conv_type=conv_type, refine=True)

        self.sga11 = SGABlock(conv_type=conv_type, channels=48, refine=True)
        self.sga12 = SGABlock(conv_type=conv_type, channels=48, refine=True)
        self.sga13 = SGABlock(conv_type=conv_type, channels=48, refine=True)
        self.sga14 = SGABlock(conv_type=conv_type, channels=48, refine=True)
...
class GANet(nn.Module):
    def __init__(self, conv_type, maxdisp=192):
        ...
        self.cost_agg = CostAggregation(maxdisp=self.maxdisp, conv_type=self.conv_type)
