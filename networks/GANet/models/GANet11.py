    ...
    def __init__(self, in_channels, out_channels, deconv=False, is_3d=False, bn=True, relu=True, conv_type="FwSC", **kwargs):
        ...
        if is_3d:
            if deconv:
                ...
                self.conv = __convolutions__[conv_type](in_channels= in_channels, out_channels= out_channels, bias=False, **kwargs)
...
class Conv2x(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_type=None , deconv=False, is_3d=False, concat=True, bn=True, relu=True):
...
class CostAggregation(nn.Module):
    def __init__(self, conv_type , maxdisp=192):
        ...
        self.sga1 = SGABlock(conv_type=self.conv_type, refine=True)
        self.sga2 = SGABlock(conv_type=self.conv_type, refine=True)

        self.sga11 = SGABlock(conv_type=self.conv_type, channels=48, refine=True)
        self.sga12 = SGABlock(conv_type=self.conv_type, channels=48, refine=True)
