...
'''
this code has be modified to calculate computational complexity for custom layers in GANet
'''
import sys
...
def get_model_complexity_info(model, input_res,
    ...
    if input_constructor:
        print(f"Dummy forward pass for model to count FLOPs.....")
...
def compute_average_flops_cost(self):
    ...
    for module in self.modules():
        # print("<<<<<<<<<<<< {} >>>>>>>>>".format(type(module)))

        if is_supported_instance(module):
            flops_sum += module.__flops__
            # print("<<<<<<<<<<<< {} >>>>>>>>>".format(module.__flops__))
...
def relu_flops(module, input, output):
    active_elements_count = output.numel()
    module.__flops__ += int(active_elements_count)
...
def deconvNd_flops_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    input = input[0]
    ...
    active_elements_count = batch_size * np.prod(input_dims)
    overall_conv_flops = conv_per_position_flops * active_elements_count
    bias_flops = 0
    if conv_module.bias is not None:
        output_height, output_width = output.shape[2:]
...
def SGA_flops_counter_hook(module, x, y):
    # print ("%%%%%%%%%%%%%%%%%%%%%%%%%%%in SGA")
    # print(x[0].shape)
    input = x[0]
    num, channels, depth, height, width = input.shape
    flops_down_and_up = channels * width * (
                2 * (height * (2 + (depth * 10))))  # in sga_kernel_forward channels*width is made parallel in threads
    flops_right_and_left = channels * height * (
                2 * (width * (depth * 10)))  # in sga_kernel_forward channels*height is made parallel in threads
    # print(flops_down_and_up+flops_right_and_left)
    module.__flops__ += flops_down_and_up + flops_right_and_left


def LGA_flops_counter_hook(module, x, y):
    # print ("%%%%%%%%%%%%%%%%%%%%%%%%%%%in LGA")
    input= x[0]
    print(input.shape)
    num, depth, height, width = input.shape
    flops =  depth * height * width* (2 * (2 * module.radius * (2 * module.radius * 5)))
    module.__flops__ += 2 * flops  # multipied by 2 coz lga_cuda_backward is called twice in LGA2 module


# TODO: verify the accuracy
# def count_upsample(m, x, y):
#     if m.mode not in ("nearest", "linear", "bilinear", "bicubic", "trilinear"):  # "trilinear"
...
MODULES_MAPPING = {
    ...
    # activations
    torch.nn.functional.relu: relu_flops,
    ...
    # BNs
    libs.sync_bn.modules.sync_bn.BatchNorm2d: bn_flops_counter_hook,
    libs.sync_bn.modules.sync_bn.BatchNorm3d: bn_flops_counter_hook,
    ...
    torch.nn.functional.upsample: upsample_flops_counter_hook,
    # Deconvolution
    torch.nn.ConvTranspose2d: deconv_flops_counter_hook,
    torch.nn.ConvTranspose3d: deconvNd_flops_counter_hook,

    # SGA
    libs.GANet.modules.SGA: SGA_flops_counter_hook,

    # LGA
    libs.GANet.modules.LGA2: LGA_flops_counter_hook,
