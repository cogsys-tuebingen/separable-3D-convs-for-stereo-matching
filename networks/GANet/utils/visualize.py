...
def map_to_range(input,min=0,max=1):
    """
    To scale the input to given range (min and max)
    Default: [0,1]
    Args:
    ...
    Returns: scaled input in range [min, max] is returned as output
...
def group_color(est_disp, gt_disp=None, left_image=None, right_image=None, save_path=None):
    ...
    if gt_disp is not None:
        est_disp_c = disp_to_color(est_disp) / 255.0
        gt_disp_c = disp_to_color(gt_disp) / 255.0
        group_image = np.concatenate((est_disp_c, gt_disp_c), 0)
